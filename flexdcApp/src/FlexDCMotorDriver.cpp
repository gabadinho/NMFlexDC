...
FlexDCController::FlexDCController(const char *portName, const char *asynPortName, int numAxes, double movingPollPeriod, double idlePollPeriod)
    ...
    createParam(AXIS_HOMRMACRO_PARAMNAME, asynParamOctet, &driverHomeReverseMacro);
    createParam(AXIS_HOMFMACRO_PARAMNAME, asynParamOctet, &driverHomeForwardMacro);
...
asynStatus FlexDCAxis::move(double position, int relative, double minVelocity, double maxVelocity, double acceleration) {
    ...
    // check for: macro running!

    if (this->motionStatus!=0) {
        asynPrint(pC_->pasynUserSelf, ASYN_TRACE_FLOW, "Trying to stop ongoing motion on FlexDC %s axis %d\n", pC_->portName, this->axisNo_);
    ...
    if (status == asynSuccess) {
        ...
        // SPEED!!!
        asynPrint(pC_->pasynUserSelf, ASYN_TRACE_FLOW, "Moving FlexDC %s axis %d to %ld\n", pC_->portName, this->axisNo_, target);
...
asynStatus FlexDCAxis::home(double minVelocity, double maxVelocity, double acceleration, int forwards) {
    ...
    // check for: macro running, motor moving
...
asynStatus FlexDCAxis::poll(bool *moving) { 
    ...
    bool valid_motion_status = false;
    double rdbd, mres;
    ...
    if (status == asynSuccess) {
        ...
        if ((valid_motion_status) && (motion_status==0)) {
